  ...
  mark_config = {
    ...
    tacRebalance = {
      ...
      fillService = {
        ...
        senderAddress               = try(local.mark_config_json.tacRebalance.fillService.senderAddress, "")
      ...
      bridge = {
        slippageDbps       = try(local.mark_config_json.tacRebalance.bridge.slippageDbps, 500)
...
# - MM:  mandy-web3signer-mainnet-prod.mark.internal:9000
# - FS:  mandy-fillservice-web3signer-mainnet-prod.mark.internal:9000
...
# poller Lambda (poller_tac_only) remains.
#
...
module "db" {
  ...
  password                   = local.mark_config.db_password # Use password from MANDY_CONFIG_MAINNET
  port                       = var.db_port
