...
import { getMulticallAddress } from '../../src/helpers/contracts';
...
describe('Multicall Helper Functions', () => {
    describe('prepareMulticall', () => {
        ...
        it('should encode transaction data for a multicall with no values', () => {
            ...
            expect(result.to).to.equal(MOCK_MULTICALL_ADDRESS);
        ...
        it('should handle empty calls array', () => {
            ...
            expect(result).to.have.property('to', MOCK_MULTICALL_ADDRESS);
            expect(result.data).to.equal(expectedCalldata);
            expect(result.value).to.equal('0');
        });
        ...
        it('should use chain-specific address when provided', () => {
            ...
            const chainId = '123';
            const mockConfig = { chains: { '123': { deployments: { multicall3: customAddress } } } } as unknown as MarkConfiguration;

            const calls = [
                { to: '0x1234567890123456789012345678901234567890', data: '0xabcdef01' },
            ];

            const result = prepareMulticall(calls, false, chainId, mockConfig);

            expect(result.to).to.equal(customAddress);
        });
    })
}); 