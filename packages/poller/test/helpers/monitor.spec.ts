...
describe('Monitor Helpers', () => {
  ...
  beforeEach(() => {
    ...
    config = {
      chains: {
        ...
        domain2: {
          assets: [{ tickerHash: 'TICKER1', balanceThreshold: '1500' }],
          gasThreshold: '3000',
        },
      },
  ...
  describe('logBalanceThresholds', () => {
    it('should log error when balance is below threshold', () => {
      ...
      expect(logger.error.firstCall.args[0]).toBe('Asset balance below threshold');
    });

    it('should log warning when asset is not configured', () => {
      const balances = new Map([['UNKNOWN_TICKER', new Map([['domain1', BigInt(1000)]])]]);

      logBalanceThresholds(balances, config, logger);

      expect(logger.warn.calledOnce).toBe(true);
      expect(logger.warn.firstCall.args[0]).toBe('Asset not configured');
    });
    ...
    it('should handle case when balanceThreshold is not set', () => {
      ...
      const balances = new Map([['TICKER3', new Map([['domain1', BigInt(500)]])]]);
    ...
    it('should handle case when balanceThreshold is explicitly set to zero', () => {
      ...
      const configWithZeroBalanceThreshold = {
        ...
        chains: {
          domain1: {
            assets: [{ tickerHash: 'TICKER3', balanceThreshold: '0' }],
            gasThreshold: '5000',
          },
        },
      } as unknown as MarkConfiguration;

      const balances = new Map([['TICKER3', new Map([['domain1', BigInt(0)]])]]);
    ...
    it('should handle when domain has no assets configured', () => {
      ...
      const balances = new Map([['TICKER1', new Map([['domain1', BigInt(1000)]])]]);
  ...
  describe('logGasThresholds', () => {
    ...
    it('should log error when gas balance is below threshold', () => {
      ...
      const errorCall = logger.error.getCalls().find((call) => call.args[0] === 'Gas balance is below threshold');
    ...
    it('should not log when gas balance is above threshold', () => {
      ...
      const errorCalls = logger.error.getCalls().filter((call) => call.args[0] === 'Gas balance is below threshold');
    ...
    it('should log error when there is no configured gas threshold', () => {
      ...
      const gas = new Map([['domain3', BigInt(5000)]]);

      logGasThresholds(gas, configWithoutThreshold, logger);

      expect(logger.error.called).toBe(true);
      const errorCall = logger.error.getCalls().find((call) => call.args[0] === 'No configured gas threshold');
      expect(errorCall).toBeDefined();
    });
    ...
    it('should handle case when threshold is explicitly set to zero', () => {
      ...
      const gas = new Map([['domain3', BigInt(100)]]);
