...
export class LockOrBurnOutV1 {
  ...
  static layout(property?: string) {
    return borsh.struct([types.RemoteAddress.layout('destTokenAddress'), borsh.vecU8('destPoolData')], property);
  }
  ...
  static fromDecoded(obj: any) {
    return new LockOrBurnOutV1({
      ...
      destPoolData: new Uint8Array(obj.destPoolData.buffer, obj.destPoolData.byteOffset, obj.destPoolData.length),
    });
  }

  static toEncodable(fields: LockOrBurnOutV1Fields) {
    return {
      destTokenAddress: types.RemoteAddress.toEncodable(fields.destTokenAddress),
      destPoolData: Buffer.from(fields.destPoolData.buffer, fields.destPoolData.byteOffset, fields.destPoolData.length),
    };
  }
