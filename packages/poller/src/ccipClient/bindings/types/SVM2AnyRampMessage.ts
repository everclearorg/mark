...
export class SVM2AnyRampMessage {
  ...
  constructor(fields: SVM2AnyRampMessageFields) {
    ...
    this.tokenAmounts = fields.tokenAmounts.map((item) => new types.SVM2AnyTokenTransfer({ ...item }));
  ...
  static fromDecoded(obj: any) {
    return new SVM2AnyRampMessage({
      ...
      extraArgs: new Uint8Array(obj.extraArgs.buffer, obj.extraArgs.byteOffset, obj.extraArgs.length),
      feeToken: obj.feeToken,
      tokenAmounts: obj.tokenAmounts.map((item: any /* eslint-disable-line @typescript-eslint/no-explicit-any */) =>
  ...
  static toEncodable(fields: SVM2AnyRampMessageFields) {
    return {
      ...
      extraArgs: Buffer.from(fields.extraArgs.buffer, fields.extraArgs.byteOffset, fields.extraArgs.length),
      feeToken: fields.feeToken,
      tokenAmounts: fields.tokenAmounts.map((item) => types.SVM2AnyTokenTransfer.toEncodable(item)),
  ...
  static fromJSON(obj: SVM2AnyRampMessageJSON): SVM2AnyRampMessage {
    return new SVM2AnyRampMessage({
      ...
      tokenAmounts: obj.tokenAmounts.map((item) => types.SVM2AnyTokenTransfer.fromJSON(item)),
