...
export class SVM2AnyMessage {
  ...
  constructor(fields: SVM2AnyMessageFields) {
    ...
    this.tokenAmounts = fields.tokenAmounts.map((item) => new types.SVMTokenAmount({ ...item }));
  ...
  static toEncodable(fields: SVM2AnyMessageFields) {
    return {
      ...
      tokenAmounts: fields.tokenAmounts.map((item) => types.SVMTokenAmount.toEncodable(item)),
  ...
  static fromJSON(obj: SVM2AnyMessageJSON): SVM2AnyMessage {
    return new SVM2AnyMessage({
      ...
      tokenAmounts: obj.tokenAmounts.map((item) => types.SVMTokenAmount.fromJSON(item)),
