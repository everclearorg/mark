...
export class SVM2AnyTokenTransfer {
  ...
  static fromDecoded(obj: any) {
    return new SVM2AnyTokenTransfer({
      ...
      extraData: new Uint8Array(obj.extraData.buffer, obj.extraData.byteOffset, obj.extraData.length),
      amount: types.CrossChainAmount.fromDecoded(obj.amount),
      destExecData: new Uint8Array(obj.destExecData.buffer, obj.destExecData.byteOffset, obj.destExecData.length),
    });
  }
  ...
  static toEncodable(fields: SVM2AnyTokenTransferFields) {
    return {
      ...
      extraData: Buffer.from(fields.extraData.buffer, fields.extraData.byteOffset, fields.extraData.length),
      amount: types.CrossChainAmount.toEncodable(fields.amount),
      destExecData: Buffer.from(fields.destExecData.buffer, fields.destExecData.byteOffset, fields.destExecData.length),
    };
  }
