...
export class RemoteConfig {
  ...
  constructor(fields: RemoteConfigFields) {
    this.poolAddresses = fields.poolAddresses.map((item) => new types.RemoteAddress({ ...item }));
  ...
  static fromDecoded(obj: any) {
    return new RemoteConfig({
      poolAddresses: obj.poolAddresses.map((item: any /* eslint-disable-line @typescript-eslint/no-explicit-any */) =>
  ...
  static toEncodable(fields: RemoteConfigFields) {
    return {
      poolAddresses: fields.poolAddresses.map((item) => types.RemoteAddress.toEncodable(item)),
  ...
  static fromJSON(obj: RemoteConfigJSON): RemoteConfig {
    return new RemoteConfig({
      poolAddresses: obj.poolAddresses.map((item) => types.RemoteAddress.fromJSON(item)),
