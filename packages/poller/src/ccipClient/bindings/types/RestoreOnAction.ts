...
export function fromJSON(obj: types.RestoreOnActionJSON): types.RestoreOnActionKind {
  switch (obj.kind) {
    case 'None': {
      return new None();
    }
...
export function layout(property?: string) {
  const ret = borsh.rustEnum([borsh.struct([], 'None'), borsh.struct([], 'Upgrade'), borsh.struct([], 'Rollback')]);
