...
export interface RemoveOfframpArgs {
  sourceChainSelector: BN;
  offramp: PublicKey;
}

export interface RemoveOfframpAccounts {
...
export const layout = borsh.struct([borsh.u64('sourceChainSelector'), borsh.publicKey('offramp')]);
 ...
 */
export function removeOfframp(
  args: RemoveOfframpArgs,
  accounts: RemoveOfframpAccounts,
  ...
  const identifier = Buffer.from([252, 152, 51, 170, 241, 13, 199, 8]);
