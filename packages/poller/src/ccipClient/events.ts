...
export async function parseCCIPMessageSentEvent(
  ...
  try {
    ...
    logger.debug(`Looking for program return log from CCIP Router: ${routerProgramId}`);
    ...
    const programReturnLog = tx.meta.logMessages.find((log) => log.includes(`Program return: ${routerProgramId}`));

    if (programReturnLog) {
      logger.debug(`Found CCIP program return log`);

      // Extract the base64 data after the program ID
      const parts = programReturnLog.split(`Program return: ${routerProgramId} `);
      if (parts.length > 1) {
    ...
    logger.warn(`Could not find CCIP Router program return log in transaction logs`);
    return { messageId: undefined };
  } catch (error) {
    throw (
      (new Error(`Failed to parse message ID from transaction`),
      {
