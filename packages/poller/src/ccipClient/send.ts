...
import { CCIPContext, CCIPSendRequest, CCIPSendOptions, CCIPCoreConfig } from './models';
import { CCIPAccountReader } from './accounts';
import { ccipSend, CcipSendAccounts, CcipSendArgs } from './bindings/instructions/ccipSend';
import {
  findConfigPDA,
...
export async function sendCCIPMessage(
  ...
  logger.debug(`Using fee token: ${feeTokenMint.toString()} (${isNativeSol ? 'Native SOL' : 'SPL Token'})`);

  // Determine the correct fee token program ID
  let feeTokenProgramId = TOKEN_PROGRAM_ID;
  if (!isNativeSol) {
    feeTokenProgramId = await detectTokenProgram(feeTokenMint, connection, logger);
  }
  ...
  if (sendOptions?.skipPreflight) {
    ...
    try {
      ...
      logger.debug(
        `Confirmation error: ${confirmError instanceof Error ? confirmError.message : String(confirmError)}`,
      );
    }
  } else {
...
async function buildCCIPSendAccounts(
  ...
  try {
    ...
    logger.debug(`Fee token: ${feeTokenMint.toString()} (${isNativeSol ? 'Native SOL' : 'SPL Token'})`);
    ...
    logger.debug(`Deriving token accounts for fee token: ${feeTokenMint.toString()}`);
...
async function buildTokenAccountsForSend(
  ...
  for (const tokenAmount of request.tokenAmounts) {
    try {
      ...
      const tokenProgram = await detectTokenProgram(tokenMint, connection, logger);
      ...
      logger.trace(`Retrieved token admin registry for ${tokenMint.toString()}: ${JSON.stringify(tokenAdminRegistry)}`);

      // Get lookup table for this token
      const lookupTable = await getLookupTableAccount(connection, tokenAdminRegistry.lookupTable, logger);
      lookupTableList.push(lookupTable);

      // Get the lookup table addresses
      const lookupTableAddresses = lookupTable.state.addresses;

      logger.trace(`Lookup table addresses: ${JSON.stringify(lookupTableAddresses)}`);
...
async function getLookupTableAccount(
  ...
  if (!lookupTableAccount) {
    throw enhanceError(new Error(`Lookup table not found: ${lookupTableAddress.toString()}`), {
 ...
 */
function getPoolProgram(lookupTableAddresses: PublicKey[], logger: Logger): PublicKey {
  const enhanceError = createErrorEnhancer(logger);
  // The pool program is at index 2 in the lookup table
  if (lookupTableAddresses.length <= 2) {
    throw enhanceError(new Error("Lookup table doesn't have enough entries to determine pool program"), {
