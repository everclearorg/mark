...
import { CCIPContext, TokenPoolChainConfigResponse } from '../../models';
...
import { ChainConfigFields, ChainConfig } from '../../burnmint-pool-bindings/types/ChainConfig';
import { PoolConfig } from '../../burnmint-pool-bindings/types/PoolConfig';
...
export class BurnMintTokenPoolAccountReader implements TokenPoolAccountReader {
  ...
  constructor(
    ...
    this.logger = context.logger ?? createLogger('burnmint-pool-reader', { level: LogLevel.INFO });

    // Use provided program ID
    this.programId = programId;

    this.logger.debug(`BurnMintTokenPoolAccountReader initialized: programId=${this.programId.toString()}`);
  }
  ...
  async getGlobalConfigInfo(): Promise<BurnMintGlobalConfig> {
    ...
    try {
      this.logger.debug(`Fetching global config for program: ${this.programId.toString()}`);
      ...
      this.logger.trace(`Fetching account info for global config PDA: ${pda.toString()}`);
      const accountInfo = await this.context.provider.connection.getAccountInfo(pda);

      if (!accountInfo) {
        this.logger.debug(`Global config account not found at PDA: ${pda.toString()}`);
        throw new Error(`Global config not found for program: ${this.programId.toString()}`);
      }
      ...
      this.logger.trace(
        `Account data (first 32 bytes): ${Buffer.from(accountInfo.data).subarray(0, 32).toString('hex')}`,
      );
      ...
      if (!accountInfo.owner.equals(this.programId)) {
        ...
        throw new Error(`Global config account is not owned by program: ${this.programId.toString()}`);
      }

      // Decode the account data using borsh and the PoolConfig layout
      this.logger.trace(
        `Decoding global config data: discriminator=${Buffer.from(accountInfo.data).subarray(0, 8).toString('hex')}`,
      );
      const decoded = PoolConfig.layout().decode(Buffer.from(accountInfo.data).subarray(8)); // Skip 8-byte discriminator
      this.logger.trace(`Raw decoded global config data: version=${decoded.version}`);
      const globalConfig = PoolConfig.fromDecoded(decoded);

      if (!globalConfig) {
        throw new Error(`Failed to decode global config for program: ${this.programId.toString()}`);
      }
  ...
  async getPoolConfig(mint: PublicKey): Promise<BurnMintPoolConfig> {
    ...
    try {
      ...
      if (!accountInfo.owner.equals(this.programId)) {
        ...
        throw new Error(`Account is not owned by program: ${this.programId.toString()}`);
      }

      // Decode the account data using borsh and the State layout
      this.logger.trace(
        `Decoding account data: discriminator=${Buffer.from(accountInfo.data).subarray(0, 8).toString('hex')}`,
      );
      ...
      this.logger.trace('Complete pool config details:', {
        ...
        routerOnrampAuthority: poolConfig.config.routerOnrampAuthority.toString(),
        router: poolConfig.config.router.toString(),
   ...
   */
  async getChainConfig(mint: PublicKey, remoteChainSelector: bigint): Promise<TokenPoolChainConfigResponse> {
    const enhanceError = createErrorEnhancer(this.logger);

    try {
      this.logger.debug(`Fetching chain config for mint: ${mint.toString()}, chain: ${remoteChainSelector.toString()}`);
      const [pda, bump] = findBurnMintPoolChainConfigPDA(remoteChainSelector, mint, this.programId);
      ...
      this.logger.trace('Retrieved raw chain config:', {
        ...
        tokenAddress: chainConfigAccount.base.remote.tokenAddress.address.toString(),
        ...
        inboundRateLimit: {
          ...
          capacity: chainConfigAccount.base.inboundRateLimit.cfg.capacity.toString(),
        },
        outboundRateLimit: {
          enabled: chainConfigAccount.base.outboundRateLimit.cfg.enabled,
          capacity: chainConfigAccount.base.outboundRateLimit.cfg.capacity.toString(),
        },
      });
      ...
      this.logger.trace('Formatted chain config response:', {
        ...
        inboundRateLimit: {
          ...
          lastTxTimestamp: formattedConfig.base.inboundRateLimit.lastTxTimestamp,
          currentBucketValue: formattedConfig.base.inboundRateLimit.currentBucketValue,
        },
        outboundRateLimit: {
          ...
          lastTxTimestamp: formattedConfig.base.outboundRateLimit.lastTxTimestamp,
          currentBucketValue: formattedConfig.base.outboundRateLimit.currentBucketValue,
        },
      });
  ...
  private formatChainConfig(
    ...
    return {
      ...
      base: {
        decimals: chainConfigAccount.base.remote.decimals,
        poolAddresses: chainConfigAccount.base.remote.poolAddresses.map((addr) => ({
          address: Buffer.from(addr.address).toString('hex'),
        })),
        tokenAddress: {
          address: Buffer.from(chainConfigAccount.base.remote.tokenAddress.address).toString('hex'),
        },
        inboundRateLimit: {
          isEnabled: chainConfigAccount.base.inboundRateLimit.cfg.enabled,
          capacity: BigInt(chainConfigAccount.base.inboundRateLimit.cfg.capacity.toString()),
          rate: BigInt(chainConfigAccount.base.inboundRateLimit.cfg.rate.toString()),
          lastTxTimestamp: BigInt(chainConfigAccount.base.inboundRateLimit.lastUpdated.toString()),
          currentBucketValue: BigInt(chainConfigAccount.base.inboundRateLimit.tokens.toString()),
        },
        outboundRateLimit: {
          isEnabled: chainConfigAccount.base.outboundRateLimit.cfg.enabled,
          capacity: BigInt(chainConfigAccount.base.outboundRateLimit.cfg.capacity.toString()),
          rate: BigInt(chainConfigAccount.base.outboundRateLimit.cfg.rate.toString()),
          lastTxTimestamp: BigInt(chainConfigAccount.base.outboundRateLimit.lastUpdated.toString()),
          currentBucketValue: BigInt(chainConfigAccount.base.outboundRateLimit.tokens.toString()),
        },
      },
    };
  }
   ...
   */
  async getRateLimitConfigForChain(mint: PublicKey, remoteChainSelector: bigint): Promise<TokenPoolRateLimit> {
    try {
      ...
      this.logger.error(`Failed to get rate limit for chain ${remoteChainSelector.toString()}: ${error}`);
      throw error;
    }
  }
   ...
   */
  async getRateLimitConfigsForChains(mint: PublicKey, remoteChainSelectors: bigint[]): Promise<TokenPoolRateLimit[]> {
    ...
    try {
      ...
      const chainConfigs = await this.listChainConfigs(mint, remoteChainSelectors);
  ...
  async listChainConfigs(
    ...
    try {
      ...
      this.logger.debug(`Checking ${remoteChainSelectors.length} chain selectors`);
      ...
      for (const remoteChainSelector of remoteChainSelectors) {
        try {
          ...
          this.logger.trace(`Error checking chain config for selector ${remoteChainSelector.toString()}: ${error}`);
        }
      }
