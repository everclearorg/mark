...
export class PoolSignerNotInMultisig extends Error {
  ...
  readonly msg = 'Token Pool Signer PDA must be m times a signer of the Multisig';

  constructor(readonly logs?: string[]) {
    super('6006: Token Pool Signer PDA must be m times a signer of the Multisig');
  }
}
...
export class InvalidMultisigThreshold extends Error {
  ...
  readonly msg = 'Invalid multisig threshold: required signatures cannot exceed total signers';

  constructor(readonly logs?: string[]) {
    super('6010: Invalid multisig threshold: required signatures cannot exceed total signers');
  }
}
...
export class InvalidMultisigThresholdTooHigh extends Error {
  ...
  readonly msg = 'Invalid multisig m: required signatures cannot exceed the available for outside signers';

  constructor(readonly logs?: string[]) {
    super('6011: Invalid multisig m: required signatures cannot exceed the available for outside signers');
  }
}
