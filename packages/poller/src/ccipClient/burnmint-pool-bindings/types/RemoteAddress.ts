...
export class RemoteAddress {
  ...
  static fromDecoded(obj: any) {
    return new RemoteAddress({
      address: new Uint8Array(obj.address.buffer, obj.address.byteOffset, obj.address.length),
    });
  }

  static toEncodable(fields: RemoteAddressFields) {
    return {
      address: Buffer.from(fields.address.buffer, fields.address.byteOffset, fields.address.length),
    };
  }
