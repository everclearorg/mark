...
describe('Invoice Helpers', () => {
  ...
  describe('checkPendingInvoices', () => {
    ...
    it('should skip invoices marked as invalid', async () => {
      const invoices: Invoice[] = [
        {
          intent_id: 'invoice-1',
          amount: '1000',
      ...
      mockEventQueue.isInvalidInvoice.mockResolvedValue(true);
      mockEventQueue.hasEvent.mockResolvedValue(false);

      await checkPendingInvoices(mockAdapters);

      expect(mockEventQueue.isInvalidInvoice).toHaveBeenCalledWith('invoice-1');
