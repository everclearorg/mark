  ...
  return withTransaction(async (client) => {
    const query = `
      INSERT INTO rebalance_operations (
        invoiceId, originChainId, destinationChainId, tickerHash,
...
export async function getPendingOperations(
  ...
  try {
    let query = `
      SELECT
        id, invoiceId, originChainId, destinationChainId, tickerHash,
    ...
    if (filters.tickerHash) {
      query += ` AND tickerHash = $${++paramCount}`;
      params.push(filters.tickerHash);
    }
