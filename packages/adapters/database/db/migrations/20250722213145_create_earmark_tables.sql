...
CREATE INDEX idx_earmarks_chain_ticker ON earmarks(destinationChainId, ticker);
...
COMMENT ON COLUMN earmarks.ticker IS 'Token ticker (e.g., USDC, ETH) required for invoice payment';
...
COMMENT ON COLUMN rebalance_operations.slippage IS 'Expected slippage for this rebalancing operation';
COMMENT ON COLUMN rebalance_operations.status IS 'Operation status: pending, in_progress, completed, failed';
...
DROP TABLE IF EXISTS earmark_audit_log;
