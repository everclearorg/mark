...
// Get mocked GetExecutionStatusResponse
const MockGetExecutionStatusResponse = (jest.requireMock('@defuse-protocol/one-click-sdk-typescript') as any)
  .GetExecutionStatusResponse;

// Mock deposit status response
const mockStatusResponse = {
  status: MockGetExecutionStatusResponse.status.SUCCESS,
...
describe('NearBridgeAdapter', () => {
  ...
  describe('readyOnDestination', () => {
    ...
    it('should return false if no deposit address found', async () => {
      const route: RebalanceRoute = {
        asset: mockAssets['USDC_ETH'].address,
        origin: 1,
        destination: 42161,
      ...
      jest.spyOn(adapter, 'getTransactionValue').mockResolvedValue(BigInt('1000000000000000000'));
      jest.spyOn(adapter, 'extractDepositAddress').mockReturnValue(undefined);
    ...
    it('should return false if error occurs', async () => {
      ...
      jest.spyOn(adapter, 'getTransactionValue').mockRejectedValue(new Error('Network error'));
  ...
  describe('destinationCallback', () => {
    it('should return undefined when no callback is needed', async () => {
      const route: RebalanceRoute = {
        asset: mockAssets['USDC_ETH'].address,
        origin: 1,
        destination: 42161,
      ...
      jest.spyOn(adapter, 'getTransactionValue').mockResolvedValue(BigInt('1000000000000000000'));
      jest.spyOn(adapter, 'extractDepositAddress').mockReturnValue('0xDepositAddress');
      jest.spyOn(adapter, 'getDepositStatusFromApi').mockResolvedValue(mockStatusResponse as any);
      jest.spyOn(adapter, 'requiresCallback').mockResolvedValue({ needsCallback: false });
    ...
    it('should throw error when no deposit address found', async () => {
      const route: RebalanceRoute = {
        asset: mockAssets['USDC_ETH'].address,
        origin: 1,
        destination: 42161,
      ...
      await expect(adapter.destinationCallback(route, mockReceipt as TransactionReceipt)).rejects.toThrow(
        'No deposit address found in transaction receipt',
      );
    });

    it('should throw error when transaction is not filled', async () => {
      const route: RebalanceRoute = {
        asset: mockAssets['WETH'].address,
        origin: 1,
        destination: 42161,
      ...
      jest.spyOn(adapter, 'getTransactionValue').mockResolvedValue(BigInt('1000000000000000000'));
      jest.spyOn(adapter, 'extractDepositAddress').mockReturnValue('0xDepositAddress');
      jest.spyOn(adapter, 'getDepositStatusFromApi').mockResolvedValue({
        ...mockStatusResponse,
        status: MockGetExecutionStatusResponse.status.PENDING_DEPOSIT,
      } as any);

      await expect(adapter.destinationCallback(route, mockReceipt as TransactionReceipt)).rejects.toThrow(
        'Transaction (depositAddress: 0xDepositAddress}) is not yet filled',
      );
    });

    it('should return wrap transaction for WETH origin', async () => {
      const route: RebalanceRoute = {
        asset: mockAssets['WETH'].address,
        origin: 1,
        destination: 42161,
      ...
      jest.spyOn(adapter, 'findMatchingDestinationAsset').mockReturnValue(mockAssets['WETH']);

      const result = await adapter.destinationCallback(route, mockReceipt as TransactionReceipt);

      expect(result).toBeDefined();
      expect(result?.memo).toBe(RebalanceTransactionMemo.Wrap);
      expect(result?.transaction.to).toBe(mockAssets['WETH'].address);
      expect(result?.transaction.data).toBe('0xd0e30db0'); // deposit() selector
      expect(result?.transaction.value).toBe(BigInt('1000000000000000000'));
    });

    it('should return undefined for non-WETH assets', async () => {
      const route: RebalanceRoute = {
        asset: mockAssets['USDC_ETH'].address,
        origin: 1,
        destination: 42161,
      ...
      jest.spyOn(adapter, 'getTransactionValue').mockResolvedValue(BigInt('1000000'));
      jest.spyOn(adapter, 'extractDepositAddress').mockReturnValue('0xDepositAddress');
      jest.spyOn(adapter, 'getDepositStatusFromApi').mockResolvedValue(mockStatusResponse as any);
      jest.spyOn(adapter, 'requiresCallback').mockResolvedValue({
        needsCallback: true,
        amount: BigInt('1000000'),
        asset: mockAssets['USDC_ETH'],
      });
    ...
    it('should handle errors gracefully', async () => {
      ...
      await expect(adapter.destinationCallback(route, mockReceipt as TransactionReceipt)).rejects.toThrow(
        'Failed to prepare destination callback: Network error',
      );
  ...
  describe('getTransactionValue', () => {
    it('should get transaction value from provider', async () => {
